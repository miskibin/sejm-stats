...
class CommitteeDetailSerializer(serializers.ModelSerializer):
    ...
    def setup_eager_loading(cls, queryset):
        return queryset.prefetch_related(
            Prefetch(
                "members", queryset=CommitteeMember.objects.select_related("envoy")
            ),
            Prefetch(
                "sittings",
                queryset=CommitteeSitting.objects.prefetch_related(
                    Prefetch("prints", queryset=PrintModel.objects.only("id", "title"))
                ).order_by("-date")[:5],
    ...
    def to_representation(self, instance):
        ...
        ret["members"] = sorted(
            ret["members"],
            key=lambda x: (x["function"] is None, x["function"], x["envoy_name"]),
        )
        return ret
